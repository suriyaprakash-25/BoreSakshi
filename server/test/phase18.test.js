...
function validEnv(overrides = {}) {
  return {
    ...
    MONGODB_URI: "mongodb+srv://prod-cluster.internal/BoreSakshi",
    MONGODB_DB: "BoreSakshi",
    FRONTEND_URL: "https://boresakshi.in",
    TRUST_PROXY: "1",
    ...
    CENTRAL_LOGGING_CONFIGURED: "YES",
    ALERT_DELIVERY_CONFIGURED: "YES",
...
test("production deployment configuration rejects unsafe/local or incomplete production settings", () => {
  ...
  assert.ok(result.errors.some((item) => item.includes("CENTRAL_LOGGING_CONFIGURED=YES")));
  assert.ok(result.errors.some((item) => item.includes("ALERT_DELIVERY_CONFIGURED=YES")));
});

test("production deployment configuration rejects example/placeholder secrets and endpoints", () => {
  const result = validateProductionDeploymentEnv(validEnv({
    MONGODB_URI: "mongodb+srv://CHANGE_ME_CLUSTER/BoreSakshi",
    JWT_SECRET: "CHANGE_ME_RANDOM_48_PLUS_CHARACTERS",
  }));
  assert.equal(result.ok, false);
  assert.ok(result.errors.some((item) => item.includes("MONGODB_URI still contains a placeholder")));
  assert.ok(result.errors.some((item) => item.includes("JWT_SECRET still contains a placeholder")));
});
